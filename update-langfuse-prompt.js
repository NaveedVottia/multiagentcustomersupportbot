...
const updatedPrompt = `あなたは修理サービスアシスタントです。
...
【顧客情報提供時の処理 - 重要】：
- ユーザーが会社名、メールアドレス、電話番号を提供した場合：
  1) lookupCustomerFromDatabaseツールを必ず呼び出してデータベースを検索する
  2) ツールの結果を待ってから回答を構成する
  3) 顧客が見つかった場合：「お客様の情報が見つかりました。修理サービスメニューをご案内します。」と応答
  4) 顧客が見つからない場合：「申し訳ございません、そのお客様情報が見つかりませんでした。再度お確かめの上、お知らせください。」と応答
  5) 必ずツールを呼び出して実際のデータベース検索を行う
...
【重要：ツール呼び出しルール】
- 顧客情報が提供されたら、必ずlookupCustomerFromDatabaseツールを呼び出す
- FAQキーワードが入力されたら、必ずsearchFAQDatabaseツールを呼び出す
- ツールの結果を待ってから回答を構成する
...
async function updatePrompt() {
  try {
    ...
    // First, get the current prompt to see its structure
    const currentPrompt = await langfuse.getPrompt('customer-identification', undefined, { cacheTtlSeconds: 0 });
    console.log('📝 Current prompt version:', currentPrompt.version);

    // Note: Langfuse SDK may not support direct prompt updates via API
    // This would typically be done through the Langfuse web interface
    console.log('⚠️  Langfuse prompt updates are typically done through the web interface');
    console.log('📝 Updated prompt content prepared:');
    console.log('=====================================');
    console.log(updatedPrompt.substring(0, 300) + '...');
    console.log('=====================================');

    console.log('\n📋 Next steps:');
    console.log('1. Go to Langfuse dashboard');
    console.log('2. Find the "customer-identification" prompt');
    console.log('3. Replace the content with the updated prompt above');
